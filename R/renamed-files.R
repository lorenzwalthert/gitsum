...
#' are not corrected retroactive. Hence, if one want's to process file-specific
...
#' updating the file names recursively and setting them to the latest name of
#' all files.
...
#' reassignment.
#' @param raw_reasignment Character vector with raw re-assignments.
...
#'
#' Takes a dir and the new
#' @importFrom rlang ll !!!
combine_dir_and_base_one <- function(dirname, basename, reassignment_index) {
  out <- ll(!!!as.list(file.path(dirname, basename)), reassignment_index) %>%
